...
\titleformat{\subsubsection}{\ttfamily}{\thesection}{0em}{}

\usepackage[titles]{tocloft}
\renewcommand\cfttoctitlefont{\ttfamily}
...
\setlength\cftsubsecindent{6.85em}
\setlength\cftsubsubsecindent{13.7em}

\renewcommand{\cftpnumalign}{l}
%\let\origcftsecpagefont\cftsecpagefont
%\renewcommand{\cftsecpagefont}{\thesection .\origcftsecpagefont}
%\let\origcftsubsecpagefont\cftsubsecpagefont
%\renewcommand{\cftsubsecpagefont}{\thesection .\origcftsubsecpagefont}
%\let\origcftsubsubsecpagefont\cftsubsubsecpagefont
%\renewcommand{\cftsubsubsecpagefont}{\thesection .\origcftsubsubsecpagefont}
...
\addcontentsline{toc}{subsection}{Bases de Numeração}
...
\addcontentsline{toc}{subsection}{Bases mais empregadas em computação}
...
\phantom \ \ \ \uline{seis}}
\addcontentsline{toc}{subsection}{Conversão entre as bases dois, dez \ e\\ dezesseis}
...
\addcontentsline{toc}{subsection}{Soma de números binários positivos}
...
\addcontentsline{toc}{subsection}{Representação de números negativos}
...
\addcontentsline{toc}{subsection}{Aritmética no Patinho Feio}
...
\begin{picture}(0,0)
\put(142,37){\line(1,0){56}}
\end{picture}\\[-1.5em]

Desta forma, consegue-se realizar qualquer soma e subtração,\\
no Patinho Feio, de números de oito bits representados \hfill na\\
notação complemento de dois, utilizando-se apenas a soma \hfill e\\
a complementação de dois.\\
\\[-0.5em]
Obs.: Convém repetir que nada impede que se considere, \hfill se\\
\phantom \ \ \ \ \ \ assim for conveniente, a sequência 1111 1010 como \hfill um\\
\phantom \ \ \ \ \ \ número de oito bits desprovido de sinal (positivo) que\\
\phantom \ \ \ \ \ \ valeria então (250) \ ; \ analogamente 1111 1011 \hfill vale-\\[-1em]
\phantom \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ 10\\[-0.5em]
\phantom \ \ \ \ \ \ ria (251) \ ; somando-se estes dois números, \hfill obtém-se\\[-1em]
\phantom \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ 10\\[-0.5em]
\phantom \ \ \ \ \ \ (501) \ , que em binário é 1 1111 0101; este é \hfill exata-\\[-1em]
\phantom \ \ \ \ \ \ \ \ \ \ \ 10\\[-0.5em]
\phantom \ \ \ \ \ \ mente o resultado obtido na soma, desde que se consi-\\
\phantom \ \ \ \ \ \ dere o \uline{vai-um} como nono bit da mesma.\\[-0.5em]

\item Transbordo:\\[0.5em]
Como os números foram limitados a oito bits de comprimento,\\
dos quais o primeiro é o bit de sinal, nota-se que \hfill existi-\\
rão valores de \uline a e \uline b tais que, \uline{a+b} ou \uline{a-b} seja muito grande\\
ou muito pequeno para ser representado em complemento \hfill de\\
dois em oito bits. Contudo, é sempre possível fazer a \hfill soma\\
tal como no item anterior, embora ela resulte errada. Neste\\
caso diz-se que houve \uline{transbordo} (do inglês ``overflow'').
...
\addcontentsline{toc}{subsection}{Blocos e Diagramas Lógicos}
...
Algumas funções são muito utilizadas e têm inclusive\\
representação gráfica como um bloco lógico:\\
\\
\phantom \ \ \ \ \d{o}\\[-1.2em]
\phantom \ \ \ 1 ) Função NOT (negação) \ \ \ \ bloco:\\[-1em]

\vspace{-2em}
\hskip 15.2em \begin{circuitikz}[scale=1, transform shape] \draw
...
(-1,0.65) node{x}
(1.1,0.65) node{NOT x};
\end{circuitikz}\\[-2.2em]

\hskip 2em \uline{Tabela para a função NOT}\\[-0.5em]

\setlength{\arrayrulewidth}{0.1em}
\setlength{\tabcolsep}{0.5em}
\renewcommand{\arraystretch}{1}
\hskip 3em \begin{tabular}{r|l}
&\\[-1em]
\ x \ \ & \ x$^1$\, = NOT x\\[-0.2em]
\hline
&\\[-0.8em]
...
\hskip -3em \d{o}\\[-1.2em]
\phantom \ \ \ 2 ) Função AND (e) \ \ \ \ \ bloco:\\[-1em]

\vspace{-2em}
\hskip 13.7em \begin{circuitikz}[scale=1, transform shape] \draw
(0,0) node[and port] (and) {}
(and.in 1) node[anchor=east] {x}
(and.in 2) node[anchor=east] {y}
(and.out) node[anchor=west] {x AND y};
\end{circuitikz}\\[-0.5em]

\hskip 7.5em \uline{Tabela}\\[-0.5em]

\setlength{\arrayrulewidth}{0.1em}
\setlength{\tabcolsep}{0.5em}
\renewcommand{\arraystretch}{1}
\hskip 2em \begin{tabular}{r|l}
&\\[-1em]
x \ y \ \ & \ x . y = x AND y\\
\hline
&\\[-0.8em]
...
\hskip -1.5em x\,.\,y \,só é \uline{um} quando x \uline e y forem ambos \uline{um}.\\[1em]

\hskip -3em \d{o}\\[-1.2em]
\phantom \ \ \ 3 ) Função OR (ou) \ \ \ \ \ bloco:\\[-1em]
...
(or.out) node[anchor=west] {x OR y};
\end{circuitikz}\\[-1em]
...
\hskip -1.5em x\,+\,y \,vale \uline{um} quando pelo menos uma das variáveis valer\par
\hskip -1.5em \uline{um}.\\[1em]

\hskip -3em \d{o}\\[-1.2em]
\phantom \ \ \ 4 ) Função NAND \ \ \ \ \ \ bloco:\\
\phantom \ \ \ \ \ \ \ É um AND,seguido de um NOT.\\[-4.5em]

\hskip 12.5em \begin{circuitikz}[scale=1, transform shape] \draw
(0,0) node[nand port] (nand) {}
(nand.in 1) node[anchor=east] {x}
(nand.in 2) node[anchor=east] {y}
(nand.out) node[anchor=west] {x NAND y};
\end{circuitikz}\\[-1em]
...
\hskip -1.5em (x.y)$^1$\, vale um quando as duas variáveis não forem \hfill si-\par
\hskip -1.5em multaneamente \uline{um}.\\

\hskip -3em \d{o}\\[-1.2em]
\phantom \ \ \ 5 ) Função NOR \ \ \ \ \ bloco:\\
\phantom \ \ \ \ \ \ \ É um OR,seguido de um NOT.\\[-4.5em]

\hskip 11.5em \begin{circuitikz}[scale=1, transform shape] \draw
(0,0) node[nor port] (nor) {}
(nor.in 1) node[anchor=east] {x}
(nor.in 2) node[anchor=east] {y}
(nor.out) node[anchor=west] {x NOR y};
\end{circuitikz}\\

\hskip 7em \uline{Tabela}\\[-0.5em]

\setlength{\arrayrulewidth}{0.1em}
\setlength{\tabcolsep}{0.5em}
\renewcommand{\arraystretch}{1}
\hskip 1em \begin{tabular}{r|l}
&\\[-1em]
x \ y \ & \ (x+y)$^1$\, = x NOR y\\
\hline
&\\[-0.8em]
...
\hskip -1.5em (x+y)$^1$\, só vale \uline{um} se x e y forem ambos \uline{zero}.\\[1em]

\hskip -3em \d{o}\\[-1.2em]
\phantom \ \ \ 6 ) Função XOR (ou exclusivo) \ \ \ \ bloco:\\[-3em]

\hskip 16.5em \begin{circuitikz}[scale=1, transform shape] \draw
(0,0) node[xor port] (xor) {}
(xor.in 1) node[anchor=east] {x}
(xor.in 2) node[anchor=east] {y}
(xor.out) node[anchor=west] {x XOR y};
\end{circuitikz}\\[-0.5em]
...
\hskip -1.5em x\,\,\textcircled{+}\,\,y vale \uline{um} quando x tiver um valor diferente de y.\\[2em]

\hskip -5.5em c) Diagramas lógicos:\\

\noindent \ \ Um diagrama lógico é um conjunto de blocos lógicos interli-\\
\phantom \ \ gados de forma a constituir uma função lógica.\\
